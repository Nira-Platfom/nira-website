...
import { Plus, Search, Grid3x3, List, Package, Pencil, Trash2, AlertTriangle, Copy, Upload, Download, CheckCircle2 } from "lucide-react";
...
function BulkImportModal({ open, onClose, onImported }: { open: boolean; onClose: () => void; onImported: () => void }) {
  ...
  const downloadTemplate = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = "nira_products_template.csv";
      a.click();
  ...
  const handleFile = async (file: File | undefined) => {
    ...
    try {
      ...
      fd.append("file", file);
      const { data } = await api.post("/products/bulk-import", fd, {
  ...
  return (
    <Modal open={open} onClose={close} title="Import Products">
      <input
        ref={inputRef}
        type="file"
        accept=".csv,text/csv"
      ...
      {!result ? (
        <div className="space-y-3">
          <p className="text-sm text-slate-500">
            Add many products at once from a spreadsheet. Download the template, fill in your products, then upload
            it back here.
          </p>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[12px] text-slate-500 mt-0.5">CSV only — up to 500 products per file</p>
            </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-4 rounded-lg bg-mint-light">
            ...
            <p className="text-sm text-charcoal">
              <strong className="font-medium">{result.created}</strong> product{result.created === 1 ? "" : "s"} added
